...
export default function XSushi() {
  ...
  const xSushi1d = useTokens({
    chainId,
    variables: { block: block1d, where: { id: sRADIO[chainId].address.toLowerCase() } },
  })?.[0]
    ...
    <AnalyticsContainer>
      ...
      <div className="pt-4 space-y-5 lg:px-14">
        <div className="flex flex-row space-x-4 overflow-auto">
          ...
          <InfoCard text="sRADIO Supply" number={formatNumber(bar?.totalSupply)} />
          <InfoCard text="sRADIO : RADIO" number={Number(bar?.ratio ?? 0)?.toFixed(4)} />
        </div>
