...
export default function LiquidityHeader({ input = undefined, output = undefined }: any): JSX.Element {
  ...
  return (
    <div className="grid grid-cols-2 md p-3px bg-dark-800">
      <NavLink
        activeClassName="font-bold text-high-emphesis bg-dark-900"
        href={`/add/${currencyId(input)}/${currencyId(output)}`}
      >
        <a className="flex items-center justify-center px-4 py-3 text-base font-medium text-center md md:px-10 text-secondary hover:text-high-emphesis">
          Add
        </a>
      </NavLink>
      <NavLink
