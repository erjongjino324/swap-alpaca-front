...
export default function ListToggle({ id, isActive, bgColor, toggle }: ToggleProps) {
  return (
    <div
      ...
      } full flex items-center outline-none cursor-pointer border-none py-1 px-3 space-x-3`}
      onClick={toggle}
    >
      {isActive && <div className="my-1.5 font-semibold">ON</div>}
      <div className={`full w-5 h-5 ${!isActive && 'bg-dark-700'}`} style={{ backgroundColor: isActive && bgColor }} />
